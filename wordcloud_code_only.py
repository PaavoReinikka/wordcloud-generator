#!/usr/bin/env python3
"""
Generate a wordcloud from only code files (no docs, markdown, etc).
...
def collect_words(base_dir, exclude_patterns):
    """Collect all words from code files only."""
    words = []
    # Only code files - no markdown, txt, json, yaml, html, css
    code_extensions = {'.py', '.js', '.ts', '.tsx', '.jsx', '.java', '.go', '.cs', 
                       '.cpp', '.c', '.h', '.hpp', '.rs', '.rb', '.php', '.scala', 
                       '.kt', '.swift', '.m', '.sh', '.bash', '.pl', '.r'}
    ...
    print(f"Code files processed: {files_processed}")
...
def filter_stopwords(word_counts):
    ...
    programming_keywords = {
        'var', 'let', 'const', 'def', 'class', 'return', 'import', 'function',
        'if', 'else', 'elif', 'then', 'end', 'do', 'while', 'for', 'try', 'catch',
        'finally', 'throw', 'new', 'this', 'self', 'super', 'true', 'false',
        'none', 'null', 'undefined', 'void', 'public', 'private', 'protected',
        'static', 'final', 'abstract', 'interface', 'extends', 'implements',
        'package', 'namespace', 'using', 'include', 'define', 'ifdef', 'endif',
        'extern', 'typeof', 'instanceof', 'async', 'await', 'yield', 'lambda',
        'del', 'pass', 'break', 'continue', 'raise', 'except', 'assert',
        'switch', 'case', 'default', 'goto', 'struct', 'enum', 'union',
        'typedef', 'sizeof', 'const', 'volatile', 'inline', 'virtual',
    }
    
    all_stopwords = english_stopwords | programming_keywords
    return {word: count for word, count in word_counts.items() if word not in all_stopwords}
...
def main():
    ...
    print("Collecting words from CODE files only...")
    ...
    for word, count in sorted(filtered_counts.items(), key=lambda x: x[1], reverse=True)[:50]:
        print(f"  {word}: {count}")
